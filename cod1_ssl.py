...
def jugador_yellow(data):
	global robot5,robot6,robot7,robot8,robot9
	for i in range(0, len(data.robots_yellow)):
		id_robots = data.robots_yellow[i].robot_id
		if id_robots == 0:
			robot5 = data.robots_yellow[i]
		if id_robots == 1:
			robot6 = data.robots_yellow[i]
		if id_robots == 2:
			robot7 = data.robots_yellow[i]
		if id_robots == 3:
			robot8 = data.robots_yellow[i]
		if id_robots == 4:
			robot9 = data.robots_yellow[i]
...
if __name__=="__main__":
	...
	while not rospy.is_shutdown():
		...
		if(pball != (0,0)):
			...
			yball = pball[1]
		print(dist())
		distance = round(math.hypot(xball - robot3.x, yball - robot3.y),2)
		go_to_target(xball,yball,robot3.x,robot3.y,robot3.orientation,0.2,0.2,2,0.3,100,pub4)
		if(distance < 100):
			shootout(pub4)
		print("Distancia",  distance)

		goalkeeper(-100,500,50,-500,0,robot2.orientation,robot2.x,robot2.y,pub2)
